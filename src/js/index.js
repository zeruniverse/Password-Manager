...
function dataReady(data){
    ...
    $("#pinloginform").on('submit',function(e){
        ...
        $("#pinlogin").val("Wait");
        pin=$("#pin").val();
        $.post("getpinpk.php",{user:getcookie('username'),device:getcookie('device'),sig:String(CryptoJS.SHA512(String(CryptoJS.SHA512(pin+localStorage.pinsalt))+randomLoginStamp))},function(msg){
            ...
            confkey=decryptchar(localStorage.en_login_conf,pin+msg)
                $.post("check.php",{pwd:String(CryptoJS.SHA512(String(CryptoJS.SHA512(pbkdf2_enc(pwdsk,JSsalt,500)+getcookie('username'))) + randomLoginStamp)),  user: getcookie('username')},function(msg){
    ...
    $("#loginform").on('submit',function(e){ 
        ...
        function process(){
            ...
            $.post("check.php",{pwd:String(CryptoJS.SHA512(String(CryptoJS.SHA512(login_sig+user)) + randomLoginStamp)),  user: user},function(msg){
                $(".errorhint").hide();
                if(msg==0){
                    $("#nouser").show();
